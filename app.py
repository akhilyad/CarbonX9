...
def main():
    ...
    if page == "Calculate Emissions":
        ...
        if st.button("Optimize Route") and distance_km > 0:
            try:
                best_option, min_co2, breakdown, distances, current_co2 = optimize_route(source_country, source_city, dest_country, dest_city, distance_km, weight_tons, prioritize_green)
                mode1, ratio1, mode2, ratio2 = best_option
                co2_1, co2_2 = breakdown
                dist1, dist2 = distances
                savings = current_co2 - min_co2
                ...
                with tab3:
                    ...
                    fig.add_trace(go.Pie(
                        labels=[mode1, mode2] if mode2 else [mode1],
                        values=[co2_1, co2_2] if mode2 else [co2_1],
        ...
        with col2:
            ...
            car_miles_equivalent = co2_savings_kg / 0.4  # 0.4 kg CO₂/mile
        ...
        with tab2:
            try:
                ...
                if not packaging.empty:
                    fig = px.line(packaging.groupby('timestamp')['co2_kg'].sum().reset_index(),
                                 x='timestamp', y='co2_kg', title="Packaging Emissions Over Time")
                    st.plotly_chart(fig, use_container_width=True)
                else:
